...
if ( has_post_thumbnail() || ( ( ! empty( $properties_images ) && count( $properties_images ) ) && ( 'yes' != $prop_detail_login || is_user_logged_in() ) ) ) {
    ...
    <div class="slider-main-wrapper">
		...
		if ( isset( $_GET['slider-type'] ) ) {
			$gallery_slider_type = sanitize_text_field( $_GET['slider-type'] ); // to override for demo
		}

		if ( 'thumb-on-bottom' === $gallery_slider_type ) {
			include( 'slider-two.php' );    // slider with thumbs on bottom.
		} else {
			include( 'slider-one.php' );    // slider with thumbs on right.
		}
